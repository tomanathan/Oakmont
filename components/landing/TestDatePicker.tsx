...
export function TestDatePicker({ subskillCount }: { subskillCount: number }) {
  ...
  return (
    <div className="mt-4 grid items-center gap-6 relative overflow-hidden rounded-lg bg-gradient-to-br from-forest-600 via-forest to-forest-900 p-6 text-ivory shadow-[0_20px_50px_-28px_rgba(20,34,25,0.9)] ring-1 ring-brass/40 sm:p-8 md:grid-cols-[1fr_1.1fr]">
      <div>
          ...
          {dates.map((d) => (
            <button
              ...
              className={`rounded-full px-3.5 py-1.5 text-sm font-medium transition-colors ${
                selected === d.date ? "bg-[linear-gradient(180deg,#f0dca0,#c9a256)] text-forest-900" : "bg-white/10 text-ivory ring-1 ring-brass-light/25 hover:bg-white/20"
              }`}
            >
              {d.label}
            </button>
