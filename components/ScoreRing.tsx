...
export function ScoreRing({ score, total }: { score: number; total: number }) {
  ...
  const r = 34;
  const c = 2 * Math.PI * r;
  const color = pct === 100 ? "#c9971b" : pct >= 50 ? "#2f6f4f" : "#5f8fb8";
  return (
    <div className="relative h-[88px] w-[88px] flex-shrink-0">
      <svg viewBox="0 0 80 80" className="h-full w-full -rotate-90" aria-hidden="true">
        <circle cx="40" cy="40" r={r} fill="none" stroke="#ebe3d3" strokeWidth="7" />
        <circle
          cx="40"
          cy="40"
          r={r}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <span className="mt-1 text-[11px] text-stone-500 tabular-nums">
          {score}/{total}
        </span>
      </div>
    </div>
