...
export async function POST(req: NextRequest) {
  ...
  const link = parentId
    ? await prisma.parentLink.findUnique({
        ...
        select: { parent: { select: { passwordHash: true } }, student: { select: { firstName: true } } },
      })
    : null;
  if (!link) return NextResponse.json({ error: "Parent not found." }, { status: 404 });
  if (link.parent.passwordHash) return NextResponse.json({ error: "They've already set up their account." }, { status: 400 });
